    ...
    <Card className="rounded-3xl">
      <CardContent className="space-y-3 p-4">
        <div className="flex items-center justify-between">
          ...
          <Badge className={cn('rounded-xl border-0 px-3 py-1', unread > 0 ? 'bg-rose-50 text-rose-700' : 'bg-muted/40 text-muted-foreground')}>
            {unread} unread
          </Badge>
        </div>
        ...
        {lastEvent && lastEvent.type !== 'HEARTBEAT' && (
          <div className="rounded-2xl bg-primary/10 p-3 text-xs">
            ...
            <p className="text-muted-foreground">{lastEvent.title}: {lastEvent.message}</p>
          </div>
        )}

        <div className="space-y-2">
          {notifications.slice(0, 8).map((n) => (
            <div key={n.id} className="flex items-start justify-between gap-3 rounded-2xl border border-border/60 bg-white p-3">
